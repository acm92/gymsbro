<?php  
	...
	if(isset($agregar) && isset($cantidad)){
		...
		alert("Se ha agregado al carrito",1,'pantalones');
	}

	if(isset($busq)){
	$q = $mysqli->query("SELECT * FROM pantalones WHERE name like '%$busq%'");
	
	}else{
		$q = $mysqli->query("SELECT * FROM pantalones ORDER BY id DESC");
	}
	...
	while($r=mysqli_fetch_array($q)){
		?>
			<div class="producto">
				...
				<div> 
					<img class="img_producto" src="productos/pantalones/<?=$r['imagen']?>"/>
				</div>
...
<script type="text/javascript">
	...
	function agregar_carrito(id_producto, talla) {
			...
			var tallaPanta = opcion.options[opcion.selectedIndex].value;

			if(cantid.length>0) {
				window.location = "?p=zapatillas&agregar="+id_producto+"&cantidad="+cantid+"&caracteristica="+tallaPanta;
			}
	}
</script>