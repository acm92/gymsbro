<?php  
	...
	if(isset($agregar) && isset($cantidad)){
		...
		alert("Se ha agregado al carrito",1,'ropa');
	}

	if(isset($busq)){
	$q = $mysqli->query("SELECT * FROM ropa WHERE name like '%$busq%'");
	
	}else{
		$q = $mysqli->query("SELECT * FROM ropa ORDER BY id DESC");
	}
	...
	while($r=mysqli_fetch_array($q)){
		?>
			<div class="producto">
				...
				<div> 
					<img class="img_producto" src="productos/ropa/<?=$r['imagen']?>"/>
				</div>
...
<script type="text/javascript">
	...
	function agregar_carrito(id_producto, talla) {
			...
			if(cantid.length>0) {
				window.location = "?p=ropa&agregar="+id_producto+"&cantidad="+cantid+"&caracteristica="+tallaZapas;
			}
	}
</script>