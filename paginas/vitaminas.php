<?php  
	...
	if(isset($agregar) && isset($cantidad)){
		...
		alert("Se ha agregado al carrito",1,'vitaminas');
	}

	if(isset($busq)){
	$q = $mysqli->query("SELECT * FROM vitaminas WHERE name like '%$busq%'");
	
	}else{
		$q = $mysqli->query("SELECT * FROM vitaminas ORDER BY id DESC");
	}
	...
	while($r=mysqli_fetch_array($q)){
		?>
			<div class="producto">
				...
				<div> 
					<img class="img_producto" src="productos/vitaminas/<?=$r['imagen']?>"/>
				</div>
...
<script type="text/javascript">
	...
	function agregar_carrito(id_producto) {
			...
			if(cantid.length>0) {
				window.location = "?p=vitaminas&agregar="+id_producto+"&cantidad="+cantid+"&caracteristica="+caracteristica;
			}
	}
</script>
